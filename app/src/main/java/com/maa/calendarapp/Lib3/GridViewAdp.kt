...
class GridViewAdp(var context: Context, var list : MutableList<RowEvent>, var interval : Int,var adapter : RecyclerAdapter2?) : RecyclerView.Adapter<GridViewAdp.Holder>(){
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        val v = LayoutInflater.from(context).inflate(R.layout.view_layout,parent,false)
        return Holder(v)
    }
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        holder.itemView.setBackgroundColor(Color.parseColor(list.get(position).color))
        holder.txt.text = list.get(position).text
